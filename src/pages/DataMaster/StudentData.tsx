...
`;
  ...
  const handleExport = async () => {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          try {
            ...
            htmlContent += `<head><meta charset="utf-8"><!--[if gte mso 9]><xml><x:ExcelWorkbook><x:ExcelWorksheets><x:ExcelWorksheet><x:Name>Rekap Siswa</x:Name><x:WorksheetOptions><x:DisplayGridlines/></x:WorksheetOptions></x:ExcelWorksheet></x:ExcelWorksheets></x:ExcelWorkbook></xml><![endif]--></head>`;
            htmlContent += `<body><table border="1">`;
            
            rows.forEach((row) => {
              if (row.length === 0) {
                htmlContent += `<tr><td colspan="11" style="border: none; height: 20px;"></td></tr>`;
              } else if (row.length === 1) {
                htmlContent += `<tr><td colspan="11" style="font-weight: bold; font-size: 14px; background-color: #e0e7ff; height: 30px; vertical-align: middle;">${row[0]}</td></tr>`;
              } else if (row.includes("Tingkat Kelas") || row.includes("Kompetensi Keahlian") || row.includes("Rentang Usia")) {
            ...
            link.setAttribute("download", `Rekap_Peserta_Didik_${new Date().toISOString().split('T')[0]}.xls`);
    ...
    Swal.fire({
      ...
      text: `Data ${activeTab === 'aktif' ? 'Peserta Didik' : 'PD Keluar'} akan diunduh dalam format Excel.`,
      icon: "question",
      showCancelButton: true,
      confirmButtonColor: "#10b981",
      cancelButtonColor: "#d33",
      ...
      if (result.isConfirmed) {
        try {
          ...
          // Generate Excel HTML
          let htmlContent = `<html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:x="urn:schemas-microsoft-com:office:excel" xmlns="http://www.w3.org/TR/REC-html40">`;
          htmlContent += `<head><meta charset="utf-8"><!--[if gte mso 9]><xml><x:ExcelWorkbook><x:ExcelWorksheets><x:ExcelWorksheet><x:Name>Data Siswa</x:Name><x:WorksheetOptions><x:DisplayGridlines/></x:WorksheetOptions></x:ExcelWorksheet></x:ExcelWorksheets></x:ExcelWorkbook></xml><![endif]--></head>`;
          htmlContent += `<body><table border="1">`;
          ...
          const link = document.createElement("a");
          link.setAttribute("href", url);
          link.setAttribute("download", `Data_Peserta_Didik_${activeTab}_${new Date().toISOString().split('T')[0]}.xls`);
          document.body.appendChild(link);
          link.click();
          document.body.removeChild(link);
