...
const keyLabels: any = {
  ...
  reg_akta_lahir: 'No. Register Akte Lahir',
  agama_id: 'Agama',
  anak_keberapa: 'Anak Ke-',
  alamat_jalan: 'Jalan atau Kampung',
  rt: 'RT',
  rw: 'RW',
  provinsi: 'Provinsi',
  ...
  kode_pos: 'Kode Pos',
  jenis_tinggal_id: 'Tempat Tinggal',
  alat_transportasi_id: 'Alat Transportasi',
  lintang: 'Lintang',
  bujur: 'Bujur',
  nomor_telepon_rumah: 'No. Telepon Rumah',
  nomor_telepon_seluler: 'No. Handphone (WhatsApp)',
  email: 'Email Aktif',
  tinggi_badan: 'Tinggi Badan (cm)',
  berat_badan: 'Berat Badan (kg)',
  lingkar_kepala: 'Lingkar Kepala (cm)',
  jarak_rumah_ke_sekolah: 'Jarak ke Sekolah (m)',
  jarak_rumah_ke_sekolah_km: 'Jarak ke Sekolah (km)',
  waktu_tempuh_ke_sekolah: 'Waktu Tempuh (jam)',
  menit_tempuh_ke_sekolah: 'Waktu Tempuh (menit)',
  ...
  nama_wali: 'Nama Wali',
  nik_wali: 'NIK Wali',
  pekerjaan_id_wali: 'Pekerjaan Wali',
  tahun_lahir_wali: 'Tahun Lahir Wali',
  jenjang_pendidikan_wali: 'Pendidikan Wali',
  penghasilan_id_wali: 'Penghasilan Wali',
};

export default function StudentPerbaikan() {
  ...
  const fetchList = async () => {
    ...
    try {
      ...
      // Filter only SISWA tipe
      const filtered = (res || []).filter((item: any) => item.tipe === "SISWA");
      setList(filtered);
    } catch (e) {
      console.error(e);
      Swal.fire("Error", "Gagal mengambil daftar pengajuan perbaikan Siswa", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleApprove = async (id: string) => {
    const confirm = await Swal.fire({
      ...
      text: "Data Siswa akan diperbarui sesuai data yang diajukan.",
      icon: "question",
  ...
  return (
    <>
      <PageMeta
        title="Pengajuan Perbaikan Siswa | SIMAK Admin Panel"
        description="Daftar Pengajuan Perbaikan Data Mandiri Siswa"
      />
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] md:p-6 no-print">
          <div>
            <h3 className="text-lg font-semibold text-gray-800 dark:text-white/90">
              Pengajuan Perbaikan Data Siswa
            </h3>
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
              Verifikasi dan setujui perbaikan data mandiri yang diajukan oleh Siswa.
            </p>
          </div>
        </div>
                ...
                {currentItems.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={6} className="text-center py-8 text-gray-400 text-theme-sm">
                      {activeTab === "belum"
                        ? "Tidak ada pengajuan perbaikan data Siswa yang belum diverifikasi."
                        : "Tidak ada pengajuan perbaikan data Siswa yang sudah diverifikasi."}
                    </TableCell>
                  </TableRow>
                ) : (
      ...
      >
        <div className="flex justify-between items-center mb-6 border-b border-gray-100 dark:border-white/[0.05] pb-3">
          <div>
            <h3 className="text-lg font-bold text-gray-800 dark:text-white/90">Detail Perbaikan Data Siswa</h3>
        ...
        <div className="space-y-4 max-h-[250px] overflow-y-auto pr-2 custom-scrollbar">
          ...
          {selectedItem && Object.keys(selectedItem.perubahan || {})
            ...
            .map((key) => {
              ...
              let displaySebelumnya = val.sebelumnya !== null && val.sebelumnya !== undefined ? (typeof val.sebelumnya === 'boolean' ? (val.sebelumnya ? 'Ya' : 'Tidak') : String(val.sebelumnya)) : "-";
              let displayDiajukan = val.diajukan_nama || (val.diajukan !== null && val.diajukan !== undefined ? (typeof val.diajukan === 'boolean' ? (val.diajukan ? 'Ya' : 'Tidak') : String(val.diajukan)) : "-");
