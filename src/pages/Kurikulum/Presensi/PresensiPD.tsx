...
const PresensiPD: React.FC = () => {
  ...
  useEffect(() => {
    if (data.length > 0) {
      ...
      data.forEach(item => {
        initialMap[item.peserta_didik_id] = item.izin?.keterangan || "";
      });
  ...
  const handleKeteranganChange = (studentId: string, value: string) => {
    setKeteranganMap(prev => ({ ...prev, [studentId]: value }));
  };

  const handleQuickAbsent = async (studentId: string, status: "4" | "5" | "6" | "7") => {
    if (!sekolah?.sekolah_id) return;
    const currentKeterangan = keteranganMap[studentId] || "";
    try {
      Swal.fire({
        title: 'Menyimpan...',
      ...
      Swal.fire({
        icon: "success",
        title: "Berhasil",
        text: "Status presensi berhasil disimpan.",
        timer: 1000,
      ...
      Swal.fire({
        icon: "error",
        title: "Gagal",
        text: err.response?.data?.message || "Gagal menyimpan status presensi.",
      });
    }
  };
  useEffect(() => {
                        ...
                        return (
                          <TableRow key={item.peserta_didik_id} className="hover:bg-gray-50/50 dark:hover:bg-white/[0.01]">
                            ...
                            <TableCell className="px-5 py-3.5">
                              <input
                                type="text"
                                value={keteranganMap[item.peserta_didik_id] || ""}
                                onChange={(e) => handleKeteranganChange(item.peserta_didik_id, e.target.value)}
                            ...
                            <TableCell className="px-5 py-3.5">
                              <div className="flex items-center gap-1.5 flex-wrap">
                                <button
                                  onClick={() => handleQuickAbsent(item.peserta_didik_id, "7")}
                                ...
                                <button
                                  onClick={() => handleQuickAbsent(item.peserta_didik_id, "5")}
                                  className={`px-2.5 py-1 text-xs font-semibold rounded-lg border transition-all duration-150 ${
                                    isSakitActive 
                                ...
                                <button
                                  onClick={() => handleQuickAbsent(item.peserta_didik_id, "4")}
                                  className={`px-2.5 py-1 text-xs font-semibold rounded-lg border transition-all duration-150 ${
                                    isIzinActive 
                                ...
                                <button
                                  onClick={() => handleQuickAbsent(item.peserta_didik_id, "6")}
                                  className={`px-2.5 py-1 text-xs font-semibold rounded-lg border transition-all duration-150 ${
                                    isAlphaActive 
