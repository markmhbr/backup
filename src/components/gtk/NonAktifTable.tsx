...
export default function NonAktifTable({ onSelectionChange, searchTerm, completenessFilter, itemsPerPage }: NonAktifTableProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const result = await dapodikService.getGTK(itemsPerPage, searchTerm, currentPage, undefined, 'non-aktif', completenessFilter);
            ...
            {data.length > 0 ? (
              data.map((item) => (
                <TableRow key={item.ptk_id} className={`${selectedRows.includes(item.ptk_id) ? "bg-gray-50 dark:bg-white/[0.02]" : ""}`}>
                  ...
                  <TableCell className="px-5 py-4 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                    {formatDateDMY(item.tanggal_surat_tugas)}
                  </TableCell>
                  <TableCell className="px-5 py-4 text-start">
            ...
            <div className="flex gap-3 justify-end pt-2 border-t border-gray-100 dark:border-gray-800">
              ...
              <Button 
                ...
                onClick={() => {
                  ...
                  navigate(`/${role}/gtk-data/edit/${selectedItemForCompleteness.ptk_id}?tab=nonaktif`);
                }}
              >
                Lengkapi Data
              </Button>
