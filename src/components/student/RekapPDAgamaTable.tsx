...
interface RekapPDAgamaTableProps {
  searchTerm: string;
}

export default function RekapPDAgamaTable({ searchTerm }: RekapPDAgamaTableProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setIsLoading(true);
      try {
        const response = await dapodikService.getPdRekapAgama();
        ...
        console.error("Failed to fetch pd rekap agama:", error);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const filteredData = rekapData.filter(item => 
    item.agama.toLowerCase().includes(searchTerm.toLowerCase())
  ).sort((a, b) => a.agama.localeCompare(b.agama));
      ...
      <div className="max-w-full overflow-x-auto custom-scrollbar">
        <Table className="min-w-[1000px]">
          <TableHeader className="border-b border-gray-100 dark:border-white/[0.05]">
            <TableRow>
              <TableCell isHeader rowSpan={2} className="px-5 py-3 font-medium text-gray-500 text-start text-theme-xs dark:text-gray-400">Agama</TableCell>
            ...
            {isLoading ? (
              ...
              <>
                {filteredData.map((item, index) => (
                  <TableRow key={index}>
                    <TableCell className="px-5 py-4 text-start font-medium text-gray-800 dark:text-white/90">{item.agama}</TableCell>
