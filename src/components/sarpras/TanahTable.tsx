...
interface TanahTableProps {
  searchTerm: string;
  itemsPerPage: number;
}

export default function TanahTable({ searchTerm, itemsPerPage }: TanahTableProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const result = await dapodikService.getTanah(itemsPerPage, searchTerm, currentPage);
          ...
          <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
            {data.length > 0 ? data.map((item) => (
              <TableRow key={item.id_tanah}>
                ...
                <TableCell className="px-5 py-4 text-gray-500 dark:text-gray-400">{item.jenis_prasarana_id_str || 'Tanah'}</TableCell>
                <TableCell className="px-5 py-4 text-gray-500 dark:text-gray-400">{item.no_sertifikat_tanah || "-"}</TableCell>
                ...
                <TableCell className="px-5 py-4 text-gray-500 dark:text-gray-400">{item.luas || 0}</TableCell>
                <TableCell className="px-5 py-4 text-gray-500 dark:text-gray-400">{item.alamat_jalan || "-"}</TableCell>
              </TableRow>
            )) : (
              <TableRow>
                <TableCell colSpan={7} className="px-5 py-10 text-center text-gray-500 dark:text-gray-400">Tidak ada data tanah ditemukan</TableCell>
              </TableRow>
            )}
          </TableBody>
        </Table>
